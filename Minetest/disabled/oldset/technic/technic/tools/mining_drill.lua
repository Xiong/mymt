-- less nasty Android hack
-- change modes by use-ing
-- Xiong 2018 Dec 13
...
local function mining_drill_mk2_setmode(user,itemstack)
	...
	if meta["mode"]==nil then
		minetest.chat_send_player(player_name, S("Use while useing to change Mining Drill Mk%d modes."):format(2))
		meta["mode"]=0
		mode=0
	end
	mode=(meta["mode"])
...
local function mining_drill_mk3_setmode(user,itemstack)
	...
	if meta["mode"]==nil then
		minetest.chat_send_player(player_name, S("Use while useing to change Mining Drill Mk%d modes."):format(3))
		meta["mode"]=0
		mode=0
	end
	mode=(meta["mode"])
...
local function mining_drill_mk2_handler(itemstack, user, pointed_thing)
	...
	if not meta or not meta.mode or keys.use then
